...
abstract class baseTruckLoaderV2 extends BaseModel{

	/**
	 * Name of the table
	 */
	protected static $_tableName = "truck_loader_v2";
	...
	protected $last_update = "CURRENT_TIMESTAMP";
	protected $office;
	...
	function setUnload_date_full($theValue){
		...
		elseif($theValue!==null)
			$theValue = date(TruckLoaderV2::getConnection()->getDateFormatter(), strtotime($theValue));
	...
	function getLast_update($format = null){
		if($this->last_update===null || !$format)
			return $this->last_update;
		if(strpos($this->last_update, "0000-00-00")===0)
			return null;
		$dateTime = new DateTime($this->last_update);
		return $dateTime->format($format);
	}
	function setLast_update($theValue){
		if($theValue==="")
			$theValue = null;
		elseif($theValue!==null)
			$theValue = date(TruckLoaderV2::getConnection()->getTimestampFormatter(), strtotime($theValue));
		if($theValue===null){
			$pk = $this->getPrimaryKey();
			if($pk && $this->$pk===null)
				$theValue="CURRENT_TIMESTAMP";
			else{
			$theValue = "";
			}
		}
		if($this->last_update !== $theValue){
			$this->_modifiedColumns[] = "last_update";
			$this->last_update = $theValue;
		}
	}
	...
	function setOffice($theValue){
		if($theValue==="")
			$theValue = null;
		if($theValue!==null)
			$theValue = (int)$theValue;
		if($this->office !== $theValue){
			$this->_modifiedColumns[] = "office";
			$this->office = $theValue;
		}
	}
	...
	static function getTableName(){
		return TruckLoaderV2::$_tableName;
	}
	...
	static function getColumnNames(){
		return TruckLoaderV2::$_columnNames;
	}
	...
	static function hasColumn($columnName){
		return in_array(strtolower($columnName), array_map('strtolower', TruckLoaderV2::getColumnNames()));
	}
	...
	static function getPrimaryKeys(){
		return TruckLoaderV2::$_primaryKeys;
	}
	...
	static function getPrimaryKey(){
		return TruckLoaderV2::$_primaryKey;
	}

	/**
	 * Searches the database for a row with the ID(primary key) that matches
	 * the one input.
	 * @return TruckLoaderV2
	 */
	static function retrieveByPK( $thePK ){
		if($thePK===null)return null;
		$PKs = TruckLoaderV2::getPrimaryKeys();
		if(count($PKs)>1)
		...
		$q = new Query;
		$conn = TruckLoaderV2::getConnection();
		$pkColumn = $conn->quoteIdentifier($PKs[0]);
		$q->add($pkColumn, $thePK);
		$q->setLimit(1);
		return array_shift(TruckLoaderV2::doSelect($q));
	}
	...
	static function retrieveByPKs( $PK0 ){
		$conn = TruckLoaderV2::getConnection();
		$tableWrapped = $conn->quoteIdentifier(TruckLoaderV2::getTableName());
		...
		return TruckLoaderV2::fetchSingle($queryString);
	}

	/**
	 * Populates and returns an instance of TruckLoaderV2 with the
	 * first result of a query.  If the query returns no results,
	 * returns null.
	 * @return TruckLoaderV2
	 */
	static function fetchSingle($queryString){
		return array_shift(TruckLoaderV2::fetch($queryString));
	}

	/**
	 * Populates and returns an Array of TruckLoaderV2 Objects with the
	...
	static function fetch($queryString){
		...
		return TruckLoaderV2::fromResult($result);
	}

	/**
	 * Returns an array of TruckLoaderV2 Objects from the rows of a PDOStatement(query result)
	 * @return array
	 */
	 static function fromResult(PDOStatement $result){
		$objects = array();
		while($row = $result->fetch(PDO::FETCH_ASSOC)){
			$object = new TruckLoaderV2;
	...
	/**
	 * Returns an Array of all TruckLoaderV2 Objects in the database.
	...
	static function getAll($extra = null){
		$conn = TruckLoaderV2::getConnection();
		$tableWrapped = $conn->quoteIdentifier(TruckLoaderV2::getTableName());
		return TruckLoaderV2::fetch("SELECT * FROM $tableWrapped $extra ");
	}
	...
	static function doCount(Query $q){
		$conn = TruckLoaderV2::getConnection();
		$q = clone $q;
		if(!$q->getTable() || strrpos($q->getTable(), TruckLoaderV2::getTableName())===false )
			$q->setTable(TruckLoaderV2::getTableName());
	...
	static function doSelect(Query $q){
		...
		if(!$q->getTable() || strrpos($q->getTable(), TruckLoaderV2::getTableName())===false )
			$q->setTable(TruckLoaderV2::getTableName());
		return TruckLoaderV2::fromResult($q->doSelect($conn));
	}
