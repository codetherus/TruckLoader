...
//Generate the select for the truck width dropdown
function WidthList($twidth=''){
  $s = '<select id="twidth" name="twidth">';
  $s .= '<option value="NA"';
  if ($twidth == 'NA' || $twidth == '')
    $s .= ' selected';
  $s .= '>NA</option>';
...
//Generate the company/Broker list 09-13-2012
function CobrokerList($cobroker='')
{
  $s = '<select id="cobroker" name="cobroker">';
  $s.= '<option value=""';  
  if ($cobroker == "" || $cobroker == 'Unknown')
   $s .= ' selected';
  $s .= ">Unknown</option>\n";
...
function DisplayDriver($dta){
  ...
  $resp->assign("zones",             "value",   $rw->zones);  
  $resp->assign("ltwidth",           "innerHTML",WidthList($rw->twidth));
  ...
  $resp->assign("lcobroker_list", "innerHTML", CoBrokerList($rw->cobroker));
  if ($rw->loadid > 0)
...
function UpdateDriver($dta){
  ...
  compare('notes',$notes, $driver['notes']);
  compare('zones',$zones,$driver['zones']);
  compare('twidth',$twidth,$driver['twidth']);
  compare('cobroker',$cobroker,$driver['cobroker']);
...
*/
  ...
  $sql .= "dldeliverydate,dldeliverylocation,comments,notes,zones,twidth,cobroker) ";
  ...
  $sql .= "'$canada_limitations','$phone_numbers',$oid,'$truck_no','$dldeliverydate','$dldeliverylocation','$comments','$notes','$zones','$twidth','$cobroker')";
  QueryLog($user."-".$sql);
  $db->query($sql);
  $did = $db->insert_id;
...
$smarty->assign('twidth',WidthList());
$smarty->assign("cobroker_list", CoBrokerList());
