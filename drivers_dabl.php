...
function DisplayDriver($dta){
  ...
  //Query using the dabl objects.
  $q = new Query;
  $q->add('name',$name);
  $rw = Drivers::doSelect($q); //Using the dabl drivers class.
  $rw = $rw[0];//->toArray();
  $rw = $rw->toArray();
  $resp->alert(print_r($rw,true));
  return $resp;
  $id = $rw->id;
  $resp->assign("rowhash",          "value",     RowHash('drivers',$id)); //for concurrancy checks.
  $resp->assign("current_driver_id","value",     $id); //for updates.  
  $resp->assign("current_load_id",  "value",     $rw->getLoadid); //for page swaps
  $resp->assign("lcanada",          "innerHTML", YesNoList('canada',$rw->getCanada));
  $resp->assign("ltwiclist",        "innerHTML", YesNoList('twic',$rw->getTwic));
  $resp->assign("ltlength",         "innerHTML", GenerateTruckLength($rw->getTlength));
  $resp->assign("lttype",           "innerHTML", GenerateTruckType($rw->getTtype));
  $resp->assign("lpipestakes",      "innerHTML", YesNoList("pipe_stakes",$rw->getPipe_stakes));
  ...
  $resp->assign("reminderta",        "value",   GetReminders($name));
...
*/
  ...
  $sql .= "dldeliverydate,dldeliverylocation,comments,notes,zones) ";
