...
function MakeFullDate($rw)
{
	...
	$sql = "update truck_loader_v2 set unload_date_full = '$full_date' where id = $id";
	$db->query($sql);
}
...
function process()
{
	...
	$sql = "select * from truck_loader_v2";
	...
	foreach ($res as $rw)
	{
		...
		$id = $rw->id;
		$sql = "update truck_loader_v2 set unload_zip = '$zip' where id=$id";
		$db->query($sql);
		$zipcount++;
	}
  $sql = "update truck_loader_v2 set office = 'boise'";
  $db->query($sql);
