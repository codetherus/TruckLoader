...
class zipcode_class {
   ...
   var $decimals = 0;               // decimal places for returned distance
   ...
   function get_zips_in_range($zip, $range, $sort=1, $include_base) {
      ...
      $sql = "SELECT zip_code,lat, lon, city,state_prefix,location FROM zip_code ";
      ...
      if (!$r) {    // sql error
         ...
         while ($row = mysql_fetch_row($r)) {
            ...
            if ($dist <= $range) {            
							$dist = round($dist,$this->decimals);
							$zip = $row[0];
							$city = $row[3];
							$state = $row[4];
              $loc = $row[5];
							$rec = array($dist,$zip,$city,$state,$loc);
